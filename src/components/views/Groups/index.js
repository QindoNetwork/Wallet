...
export class Groups extends React.Component {
    ...
    state = {loading: 0, groupIDs: [], groupNames: [], groupsLength: 0 };

    async componentDidMount() {
      const { mnemonics, address } = this.props
      try {
        this.setState({
                        groupsLength: parseInt (await Contracts.getGroupsLength(address),10),
                      })
      } catch (e) {
      ...
      let names = []
      let length = 0
      if ( this.state.groupsLength > 0 ) {
        try {
          length = this.state.groupsLength - 1
    ...
    render() {
      ...
      if (this.state.groupsLength ===  0){
        ...
        return(
          ...
          <View style={styles.container}>
            <View style={styles.body}>
              <Text style={styles.message}>NoGroups</Text>
              <View style={styles.buttonsContainer}>
                <Button
                    children="Next"
                    onPress={() => this.props.navigation.navigate('CreateGroup',  {mnemonics: mnemonics, address: address} )} />
              </View>
            </View>
          </View>
